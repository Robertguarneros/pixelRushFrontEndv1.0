...
public class AskAQuestionActivity extends AppCompatActivity {
    ...
    public void sendQuestion(View view){
        ...
        Question question = new Question(date,title,message,username);

        Call<Void> callAskAQuestion = pixelRushService.askAQuestion(question);

        callAskAQuestion.enqueue(new Callback<Void>() {
            @Override
            public void onResponse(Call<Void> call, Response<Void> response) {

                if (response.isSuccessful()) {
                    Log.i("First Version","Question sent successfully");
                    Toast.makeText(AskAQuestionActivity.this,"Question sent successfully",Toast.LENGTH_SHORT).show();
                    finish();
                } else {
                    Log.i("First Version","Error: " + response.code() + " " + response.message());
                    Toast.makeText(AskAQuestionActivity.this,"Error"+response.message(),Toast.LENGTH_SHORT).show();
                }
            }
            @Override
            public void onFailure(Call<Void> call, Throwable t) {
                Log.i("First Version","Error: "+t.getMessage(),t);
                Toast.makeText(AskAQuestionActivity.this,"Error"+t.getMessage(),Toast.LENGTH_SHORT).show();
            }
        });
    }
